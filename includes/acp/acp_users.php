		...
		switch ($mode)
		{
			case 'overview':
				...
				if ($submit)
				{
					...
					switch ($action)
					{
						...
						case 'resetlogin':
							...
							add_log('admin', 'LOG_USER_RESET_LOGIN', $user_row['username']);
							add_log('user', $user_id, 'LOG_USER_RESET_LOGIN_USER');

							trigger_error($user->lang['USER_ADMIN_LOGIN_RESET'] . adm_back_link($this->u_action . '&amp;u=' . $user_id));
				...
				else
				{
					...
					if ($user_row['user_login_attempts'] > 0)
					{
						$quick_tool_ary += array('resetlogin' => 'RESET_LOGIN');
					}					
