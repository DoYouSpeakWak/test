	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			...
			case 'profile_info':
				...
				//-- mod: Prime View Self ---------------------------------------------------//
				$user->add_lang('mods/prime_view_self');
				$view_self_link = '<a href="' . append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=viewprofile&amp;u=' . $user->data['user_id']) . '">' . $user->lang['PRIME_VIEW_SELF'] . '</a>';
				$template->assign_var('L_PROFILE_INFO_NOTICE', $user->lang['PROFILE_INFO_NOTICE'] . ' ' . $view_self_link);
				//-- end: Prime View Self ---------------------------------------------------//
