...
function get_pm_from($folder_id, $folder, $user_id)
{
	...
	// Start From Author PM List
	$from_author = request_var('from_author', 0);
	$url_from_author = $sql_from_author = '';
	if ($from_author)
	{
		$url_from_author = '&amp;from_author=' . $from_author;
		$sql_from_author = ' AND p.author_id = ' . $from_author;
	}
	// End From Author PM List	
	// PM ordering options
	...
	$template->assign_vars(array(
		'PAGINATION'		=> generate_pagination(append_sid("{$phpbb_root_path}ucp.$phpEx", "i=pm&amp;mode=view&amp;action=view_folder&amp;f=$folder_id&amp;$u_sort_param{$url_from_author}"), $pm_count, $config['topics_per_page'], $start),
